...
#define NAME_LEN 50
#define CONTACT_LEN 20 // Used for phone number
#define PASSWORD_LEN 50
#define MAX_ADMINS 3    // Example limit for admins
#define MAX_TRANSACTIONS 100 // Example limit for transactions
#define MAX_CUSTOMERS 50 // Example limit for customers

#define DATA_FILE "parking_data.txt"

// Vehicle types for clarity
#define VEHICLE_TYPE_CAR 0
#define VEHICLE_TYPE_BIKE 1
#define VEHICLE_TYPE_JEEP 2

typedef struct {
    int spotId;
    int type; // 0: Car, 1: Bike, 2: Jeep
    int isOccupied;
    char licensePlate[LICENSE_PLATE_LEN];
    char driverName[NAME_LEN];
    char driverContact[CONTACT_LEN]; // Driver's phone number
    time_t entryTime;
} ParkingSpot;
...
typedef struct {
    ...
    int vehicleType; // 0: Car, 1: Bike, 2: Jeep
    int spotId;
    time_t entryTime;
    time_t exitTime;
    double cost;
...
typedef struct {
    char licensePlate[LICENSE_PLATE_LEN];
    char name[NAME_LEN];
    char contact[CONTACT_LEN]; // Customer's phone number
} Customer;
...
// Global structure to hold all program data
typedef struct {
...
// --- Data Loading and Saving Functions (Single File) ---
...
void initializeDefaultData() {
    ...
    systemData.carPricePerHour = 10.0;
    systemData.bikePricePerHour = 5.0;
    systemData.jeepPricePerHour = 15.0;
...
void loadAllData() {
    ...
    initializeDefaultData(); // Start with defaults, then overwrite with loaded data
...
void recordEntry() {
    ...
    do {
        printf("Enter driver phone number (exactly 11 digits): ");
...
void listParkedVehicles() {
    ...
    foundVehicles = 0; // Reset for next type
...
void calculateDailyRevenueSummary() {
    ...
    printf("Total Revenue Generated Today: %.2lf\n", currentDailyRevenue);
    printf("-----------------------------\n");
}
...
void addCustomerDetails() {
    ...
    do {
        printf("Enter Contact Info (Phone Number - exactly 11 digits): ");
...
void displayTotalAmount() {
    ...
    printf("Total Revenue Generated (All Time): %.2lf\n", grandTotalRevenue);
    printf("---------------------------\n");
}
...
int main() {
    int mainChoice;
    loadAllData(); // Load all data from the single file at startup
