...
struct vehicleInfo
{
    ...
    char driverName[15];
    char driverContact[15];
    struct Time dateTime;
};
struct vehicleInfo vehicleArray[100];
...
void currentDateAndTime()
{
    ...
    currentDateTime.min = t->tm_min;
    currentDateTime.hour = t->tm_hour;
    currentDateTime.day = t->tm_mday;
    currentDateTime.mon = t->tm_mon + 1;
    currentDateTime.year = t->tm_year + 1900;
}
...
{
    char letters[4];  // 3 letters + null terminator
    int digits;

    // 3 uppercase letters, a dash, and 4 digits
...
void registerAdmin()
{
    ...
    char temp_password[PASSWORD_LEN];
    ...
    do
    {
        ...
        if (!isValidPassword(temp_password))
        {
            printf("Invalid password. Password must be at least 8 characters long.\n");
        }
    } while (!isValidPassword(temp_password));
...
int loginAdmin()
{
    ...
    char name[NAME_LEN];
    char password[PASSWORD_LEN];
    char adminName[NAME_LEN];
    char adminPhoneNumber[CONTACT_LEN];
    char adminPass[PASSWORD_LEN];
    char adminEmail[EMAIL_LEN];
    printf("\n------- Admin Login -------\n");
    ...
    char yourName[NAME_LEN];
    int found = 0;
    while (fscanf(fp, "%s %s %s %s\n", adminName, adminPhoneNumber, adminEmail, adminPass) != EOF)
    {
        if (strcmp(adminName, name) == 0 && strcmp(adminPass, password) == 0)
        {
            found = 1;
            strcpy(yourName, adminName);
            break;
        }
    }
...
{
    int vType;
    char temp_license[LICENSE_PLATE_LEN];
    char temp_name[NAME_LEN];
    ...
    do
    {
        ...
        scanf("%s", temp_license);
        if (!isValidLicensePlate(temp_license))
        {
            printf("Invalid License Plate.\n");
        }
    } while (!isValidLicensePlate(temp_license));
    strcpy(licensePlate, temp_license);
    ...
    strcpy(driverContact, temp_contact);

    printf("%s\n", licensePlate);

    currentDateAndTime();
    FILE *fp = fopen("vehicle/data.txt", "a");

    fprintf(fp, "%d %s %s %s %d %d %d %d %d\n",
            x,
            licensePlate,
            driverName,
            driverContact, currentDateTime.hour, currentDateTime.min, currentDateTime.day, currentDateTime.mon, currentDateTime.year);
    fclose(fp);
}
...
int vehicleData(int index)
{
    ...
    FILE *fp = fopen("vehicle/data.txt", "r");
    while ((fscanf(fp, "%d %s %s %s %d %d %d %d %d", &vdata, lPlate, dName, dContact, &hour, &min, &day, &mon, &year)) != EOF)
    {
        vehicleArray[index].vType = vdata;
        vehicleArray[index].dateTime.day = day;
        vehicleArray[index].dateTime.mon = mon;
        vehicleArray[index].dateTime.year = year;
        vehicleArray[index].dateTime.hour = hour;
        vehicleArray[index].dateTime.min = min;
        strcpy(vehicleArray[index].licensePlate, lPlate);
        strcpy(vehicleArray[index].driverName, dName);
        strcpy(vehicleArray[index].driverContact, dContact);
        index = index + 1;
        // printf("%d   %s   %s   %s    %d:%d    %d-%d-%d\n", vdata, lPlate, dName, dContact, hour, min, day, mon, year);
    }
...
void display()
{
    int index = vehicleData(0);

    printf("Type:         License:       DriverName      contact      Time:        Date: \n");
    for (int i = 0; i < index; i++)
    {
        printf("  %d      %s      %s         %s    %d:%d      %d-%d-%d\n", vehicleArray[i].vType, vehicleArray[i].licensePlate, vehicleArray[i].driverName,
               vehicleArray[i].driverContact, vehicleArray[i].dateTime.hour, vehicleArray[i].dateTime.min, vehicleArray[i].dateTime.day, vehicleArray[i].dateTime.mon, vehicleArray[i].dateTime.year);
    }
}
